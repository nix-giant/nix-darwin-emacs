...
A nix overlay for stable Emacs releases on Darwin.

#+begin_quote
Originally forked from [[https://github.com/nix-community/emacs-overlay][nix-community/emacs-overlay]].
#+end_quote
...
I want to constrain only the input of =config= - Emacs Lisp file or Emacs Lisp code, not the way in which it is built.
...
orgToEl =
    orgFile:
    pkgs.runCommandLocal "org-to-el" { buildInputs = [ emacs ]; } ''
      emacs --batch -Q \
        --eval "(require 'org)" \
...
{ pkgs, ... }:
{
  environment.systemPackages = [
    (pkgs.emacsWithPackagesFromUsePackage {
      ...
      #
      # To bundle the config into the final result as the default init file, set it to true.
